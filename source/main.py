from data_loader import load_eeg_data, dbs_artifact_settings, open_json_file, select_recording, read_time_domain_data, read_lfp_data, compute_eeg_power
...
def main():
    ...
    try:
        ...
        eeg_power, power_time_axis = compute_eeg_power(eeg_data, dbs_freq_min, dbs_freq_max, dbs_duration_sec, channel="Cz")
        eeg_peak_idx, eeg_peak_s = find_eeg_peak(eeg_power, eeg_fs, power_time_axis, save_dir="plots")

        # Find DBS peak
        dbs_signal =  dbs_data["TimeDomainData"].values
        dbs_fs = dbs_data["SampleRateInHz"][0]
        dbs_peak_idx, dbs_peak_s = find_dbs_peak(dbs_signal, dbs_fs, save_dir="plots")

        # Save peak info
        save_sync_peak_info(eeg_file, dbs_file, eeg_peak_idx, eeg_peak_s, dbs_peak_idx, dbs_peak_s)
