...
[![CI](https://github.com/lenasalz/dbs-eeg-sync/actions/workflows/ci.yml/badge.svg)](https://github.com/lenasalz/dbs-eeg-sync/actions/workflows/ci.yml)
[![DOI](https://zenodo.org/badge/930767013.svg)](https://doi.org/10.5281/zenodo.20035605)
...
Run the bundled worked example on the synthetic sample data (no setup beyond installation):

```bash
python examples/run_example.py
```

It detects the synchronization artifact in both the EEG and the Percept LFP and saves a verification figure to `examples/output/example_sync.png`. The example data is fully synthetic (generated by `scripts/generate_synthetic_example_data.py`) and contains no patient-derived signals.

For validating detection algorithms against labeled ground truth, a small synthetic **benchmark dataset** (PD/epilepsy, clean/noisy, spike/drop, drift) is available — see [`benchmark/README.md`](benchmark/README.md).
